...
defined on rubiksplace.com and youcandothecube.com. I’ve contributed to the solving algorithm where the "speedcube"
algorithm assumes human input.
...
class Solve():
    ...
    def step_start(self, step, squares_to_solve):
        self.msg_user('\nStep: "{0}" Squares to solve: {1}'.format(step, squares_to_solve))

    def step_end(self, cube, step, squares_to_solve):
        unsolved_squares = self.get_unsolved_squares(cube, squares_to_solve)
        if unsolved_squares == []:
            self.msg_user('step {0} successfully solved'.format(step))
            return True
        else:
            self.msg_user('Unsolved squares of Step: ' + step + ' ' + str(unsolved_squares))
            return False
    ...
    def step1(self, cube):
        ...
        # step1
        self.step_start(step, squares_to_solve)
        ...
        return self.step_end(cube, step, squares_to_solve)

    def step2(self, cube):
        step = '2 - top layer corners'
    ...
    def step4(self, cube):
        step = '4 - permute top face edges'
        squares_to_solve = ('u01', 'u10', 'u12', 'u21')

        def get_status_item(cube, coordinates_to_check):
            unsoloved_coordinates = []
            top_face_color = cube.get_color('u11')
            ...
            for coordinate in coordinates_to_check:
                if top_face_color != cube.get_color('u' + coordinate):
                    return False
        ...
        def get_status(cube):
            ...
            for i in plan:
                ...
                coordinates = i[1]
                commands = i[2]
                if get_status_item(cube, coordinates):
        ...
        while True:
            state, reposition_commands = get_status(cube)
            if state == 4:
                break
    ...
    def step5(self, cube):
        step = '5 - permute top face corners'
        solved_corners_cnt = 0

        def solved_squares_cnt(cube, squares):
            top_color = ''
            squares_cnt = 0

            top_color = cube.get_color('u11')
            for square in squares:
                if cube.get_color(square) == top_color:
                    squares_cnt += 1
            return squares_cnt

        def position_cube(cube, squares):
            top_color = ''

            top_color = cube.get_color('u11')
            for i in range(0,3):
                if len(squares) == solved_squares_cnt(cube, squares):
                    return
                else:
                    self.move_cube('u', 'Turning to have {0} as squares {1}'.format(top_color, squares))

        #step5
        self.msg_user('\nStep: "{0}"'.format(step))

        for i in range(3):
            solved_corners_cnt = solved_squares_cnt(cube, ['u00', 'u20', 'u02', 'u22'])

            if solved_corners_cnt == 0:
                position_cube(cube, ['l20'])
            elif solved_corners_cnt == 1:
                position_cube(cube, ['u10', 'u01', 'u11', 'u21', 'u02', 'u12'])
            elif solved_corners_cnt == 2:
                position_cube(cube, ['f00'])
    ...
    def step6(self, cube):
        step = '6 - solve top layer corners'
        solved_corners_cnt = 0
        solved_corners = []
        rotate_commands = 'ri f ri b b r fi ri b b r r ui'

        def get_solved_corners(cube):
            corners = []
            cubelets_to_solve = (
        ...
        def solved_corners_are_adjacent(cube, solved_corners):
        ...
        def position_corners(cube):
            while True:
                solved_corners, solved_corners_cnt = get_solved_corners(cube)

                if solved_corners_cnt == 4:
                    return solved_corners, 'done'
                elif solved_corners_cnt == 2:
                    if solved_corners_are_adjacent(cube, solved_corners):
        ...
        solved_corners, state = position_corners(cube)
    ...
    def step7(self, cube):
        ...
        def move_edge_to_back(cube, edges):
            for edge in edges:
                if 'u01' in edge:
        ...
        def process_edges(cube, edge, commands_clockwise, commands_counter_clockwise):
            commands = ''
            move_edge_to_back(cube, edge)
            if cube.get_color('r10') == cube.get_color('f11'):
        ...
        solved_edges, solved_edges_cnt = get_solved_edges(cube)

        if solved_edges_cnt == 1:
            process_edges(cube, solved_edges, commands_rotate_clockwise, commands_rotate_counter_clockwise)
        else:
    ...
    def solve_cube(self, cube, render):
        def solve_steps(cube, render):
            if not self.step1(cube): return False
            if not self.step2(cube): return False
            if not self.step3(cube): return False
            self.step4(cube)
            self.step5(cube)
            if not self.step6(cube): return False

            # render.display_on()
            # render.draw(cube)
            if not self.step7(cube): return False

            return True

        # solve_cube
        self.log_file_init()
        success = solve_steps(cube, render)
