...
def solve (cube, render):
    ...
    plan_phase_2 = {
        'f00': 'l d li', 'f02': 'd', 'f20': 'done', 'f22': 'ri di r',
        'u00': 'li di l', 'u02': 'l d li', 'u20': 'r di ri', 'u22': 'ri di r',
        'l00': 'li d l', 'l02': 'd', 'l20': 'l di li', 'l22': 'd',
        'r00': 'ri di r', 'r02': 'ri d d r d', 'r20': 'r di ri', 'r22': 'di',
        'b00': 'r di ri', 'b02': 'di', 'b20': 'l d li', 'b22': 'di',
        'd00': 'd', 'd02': 'd', 'd20': 'f d fi', 'd22': 'di'}

    def coordinate_of_color(cube, cubelet, target_color):
        for key in cubelet:
             if target_color == cube.get_color(key):
                return key
        print('Color: {0} not found in keys: {1}'.format(target_color, cubelet))
        return None

    def rotate_cube(cube, new_front_color):
         if cube.get_color('f11') != new_front_color:
            print('Rotating cube to front color:', new_front_color)
            cube.rotate('tl')
            render.draw(cube)
            rotate_cube(cube, new_front_color)

    def alg0(cube, render, phase, plan, cubelet_keys):
        def alg0_move(cube, render, plan, colors):
            target_cubelet = cube.find_cubelet(colors)
            if target_cubelet is None: return False

            coordinate = coordinate_of_color(cube, target_cubelet, colors[0])
            ...
            if command == 'done':
                ...
                return alg0_move(cube, render, plan, colors)

        # alg0
        print('\nPhase: {0} Cubelet Target: {1}'.format(phase, cubelet_keys))

        for face in range(4):
            print('Solving face:', cube.get_color('f11'))

            colors = []
            for key in cubelet_keys:
                face_center = key[0] + '11'
                colors.append(cube.get_color(face_center))

            if not alg0_move(cube, render, plan, colors): return False
            print('Turning left...')
            cube.rotate('tl')
            render.draw(cube)
        return True

    def alg1(cube, render, phase):

        def find_top_layer_edge(cube):
            face_up_color = cube.get_color('u11')
            for face in (['f10', 'u12'], ['l10', 'u01'], ['b10', 'u10'], ['r10', 'u21']):
                edge_facing = face[0]
                edge_up = face[1]
                if cube.get_color(edge_facing) != face_up_color and cube.get_color(edge_up) != face_up_color:
                    return edge_facing
            else:
                return None

        def move_top_layer_edge(cube, edge):
            edge_color = cube.get_color(edge)
            if edge_color == cube.get_color(edge[0] + '11'):
                rotate_cube(cube, edge_color)
            else:
                print('Rotating bottom two layers of cube to form body upside down "T"...')
                cube.rotate('e d')
                render.draw(cube)
                move_top_layer_edge(cube, edge)

        # alg1
        print('\nPhase:', phase)

        # flip cube upside down
        print('Flipping cube upside down...')
        cube.rotate('tu tu')
        render.draw(cube)

        check = True
        while check:
            # find edge piece on top layer, create upside down "t" with it, move it to front
            target_edge = find_top_layer_edge(cube)
            if target_edge is None: break

            move_top_layer_edge(cube, target_edge)

            # determine edge placement algorithm
            edge_top_color = cube.get_color('u12')
            command = ''
            if edge_top_color == cube.get_color('l11'):
                command = 'ui li u l u f ui fi'
            elif edge_top_color == cube.get_color('r11'):
                command = 'u r ui ri ui fi u f'

            # if correct type of edge, place it, if not find the next
            if command != '':
                # place edge on middle row
                print('Locating edge piece:', edge_top_color)
                cube.rotate(command)
                render.draw(cube)
        return True
    ...
    # solve
    if not alg0(cube, render, '1 "top layer edges"', plan_phase_1, ['f10', 'u12']): return False
    if not alg0(cube, render, '2 "top layer corners"', plan_phase_2, ['f20', 'u22', 'r00']): return False
    if not alg1(cube, render, '3 "middle layer edges"'): return False
